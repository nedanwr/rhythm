...
- **Engine testability:** scheduler logic stays pure (track lengths + clock in, schedule times out — unit-testable without audio). Scheduler, queue, and DSP unit tests: EQ headroom math (`autoHeadroom`/`effectivePreamp`/`curvesEqual`) and the limiter worklet kernel (ceiling, lookahead delay, linked stereo reduction, release) run under vitest without an audio graph. Deliverable: a generated test corpus. Landed: generated sweep fixtures with known phase at the boundary, joined by the real engine in headless Chromium and _verified_ via `OfflineAudioContext` sample-continuity checks rather than by ear (`web/src/engine/gapless.browser.test.ts`, `make test-web-audio`). Still owed: codec-matrix fixtures (AAC, ALAC, FLAC, DSD, TrueHD samples) — the sweep corpus is PCM only, so it says nothing about AAC encoder delay/padding.
