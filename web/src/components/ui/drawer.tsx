...
/**
 * Layer the popup sits in. `DrawerContent` renders one already; compose it
 * yourself only when the drawer needs to be scoped to something other than
 * the viewport.
 */
...
/**
 * `container` scopes the drawer to an element instead of the whole viewport:
 * it portals there and positions against it, so a drawer can sit inside a
 * pane rather than over the entire window. Everything else is unchanged.
 */
...
function DrawerHeader({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn(
        "border-border flex h-16 shrink-0 flex-col justify-center gap-0.5 border-b px-5 group-data-[swipe-axis=y]/drawer-popup:text-center md:text-left",
        className
      )}
      {...props}
    />
...
function DrawerFooter({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn(
        "border-border mt-auto flex shrink-0 flex-col gap-2 border-t px-5 py-4",
        className
      )}
      {...props}
    />
...
function DrawerTitle({ className, ...props }: DrawerPrimitive.Title.Props) {
  return (
    <DrawerPrimitive.Title
      ...
      className={cn("text-heading text-sm font-medium", className)}
      {...props}
    />
  );
}
...
function DrawerDescription({
  ...
  return (
    <DrawerPrimitive.Description
      ...
      className={cn(
        "text-muted-foreground text-[10px] text-balance",
        className
      )}
      {...props}
    />
