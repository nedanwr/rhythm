...
import { crumbsFor } from "../components/Breadcrumbs";
...
describe("browseErrorMessage", () => {
  it("names the folder for a 404 and keeps the server's wording otherwise", () => {
    ...
    // An unreachable server keeps the client's own explanation.
    expect(
      browseErrorMessage(
...
describe("BrowsePage", () => {
  it("renders a listing with breadcrumbs back up the tree", async () => {
    vi.stubGlobal(
      "fetch",
    ...
    // The current folder is a label, not a link back to itself.
    expect(screen.queryByRole("link", { name: "Album" })).toBeNull();
  });
